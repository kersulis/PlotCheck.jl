...
function compare_series(submission::Plots.Series, reference::Plots.Series)
    ...
    @assert (sub_label == ref_label) "Label for series \"$(sub_label)\" does not match reference label \"$(ref_label)\"."
...
function compare_markers(submission::Plots.Series, reference::Plots.Series)
    sub_label = submission |> get_label
    submission_has_marker, reference_has_marker = has_path(submission), has_path(reference)
